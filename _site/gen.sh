...
# Dependencies for the favorite-image section (parsed from README.md).
FAVORITE_IMAGE_DEPS=(
	"scripts/gen_polario.py"
	"favorite-image/README.md"
)
...
if [[ "$BUILD" -eq 1 ]]; then
	...
	# favorite-image: execute just polario lines from README
	if should_process_dir "favorite-image" && [[ -f "favorite-image/README.md" ]]; then
		# Skip the build if neither local assets nor a remote source is available.
		# This lets users commit generated PDFs/JPGs instead of the original assets.
		_favorite_image_source="${FAVORITE_IMAGE_SOURCE:-}"
		if [[ -n "$_favorite_image_source" ]]; then
			_favorite_image_source="$(cd "$REPO_ROOT" && eval printf '%s' "$_favorite_image_source")" 2>/dev/null || true
		fi
		if [[ -z "$_favorite_image_source" && ! -d "favorite-image/assets" ]]; then
			echo "[favorite-image] Skipping: no favorite-image/assets/ and FAVORITE_IMAGE_SOURCE is not set."
			echo "  Commit generated PDFs/JPGs, or set FAVORITE_IMAGE_SOURCE to build them on the runner."
		else
			echo "[favorite-image] Running polario commands from README.md..."
			while IFS= read -r line; do
				# Trim leading whitespace
				line="${line#"${line%%[![:space:]]*}"}"
				if [[ "$line" == just\ polario* ]]; then
					if [[ "$INCREMENTAL" -eq 1 && "$REBUILD_ALL" -eq 0 ]] && ! any_changed_under "favorite-image/" && ! changed_file_matches_any "$(
						IFS=','
						echo "${FAVORITE_IMAGE_DEPS[*]}"
					)"; then
						echo "  SKIP (unchanged): $line"
						TOTAL_SKIPPED=$((TOTAL_SKIPPED + 1))
						continue
					fi

					log_file="$LOG_DIR/$(sanitize_log_name "$line").log"
					status=0
					run_just "$line" "$log_file" || status=$?
					if [[ $status -eq 0 ]]; then
						TOTAL_BUILT=$((TOTAL_BUILT + 1))
						rm -f "$log_file"
					elif [[ $status -eq 2 ]]; then
