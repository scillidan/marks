...
> Now that you know what kind of thing HTML is, you probably have a feel for what kind of thing the web was meant to do. That's one reason HTML based web apps will not be a decent replacement for real applications. [kqr, "Why Web Apps Disappoint Me"]

译：HTML是一种用于标记文档的语言。这意味着HTML是一种让作者能够指示读者的方式，例如“这部分应该作为标题阅读，这部分应该作为正文阅读，这里和那里需要强调。下面这部分是另一个文档的引用，而顶部则是一张图片。”  
在这种情况下，“读者”可以指任何试图理解文档结构的事物，无论是人类、网页浏览器、屏幕阅读器还是文档转换程序。  
现在，HTML 5本质上并没有什么不同。但是，它允许作者在标记文档时更加具体。作者现在可以这样描述：“这部分内容用于辅助页面导航，这部分是页面的页脚，而这部分包含一个视频。”当作者能够更精确地描述时，读者的工作就会变得更容易——尤其是在读者是机器的情况下；例如，一个将HTML页面转换为PDF文档的程序可能会跳过尝试包含视频。  
但我们讨论的仍然只是关于如何阅读文档的说明。HTML实际上并没有更多内容。  
既然您已经了解了HTML是什么，那么您可能对网页最初的用途也有所了解。这就是为什么基于HTML的网络应用程序无法成为真正应用程序的良好替代品的原因之一。