...
"""Compile committed LaTeX card files under latex/ into PDFs and JPGs."""
...
IMAGE_DIR_ENV = "LATEX_SOURCE"
IMAGE_DIR_FALLBACK = PROJECT_ROOT / "latex" / "assets"
...
def compile_tex(tex_path, fail_on_error=True):
    ...
    out_dir = latex_dir / "_output"
    pdfs_dir = out_dir / "pdfs"
    pdfs_dir.mkdir(parents=True, exist_ok=True)
    out_rel = pdfs_dir.relative_to(latex_dir).as_posix()
...
def main():
    ...
    parser.add_argument(
        "--out",
        default=str(PROJECT_ROOT / "latex"),
        help="Directory containing the .tex cards (default: %(default)s)",
    )
    parser.add_argument(
        "--compile",
        default=None,
        help="Compile the card for this stem or .tex path (writes latex/_output/pdfs).",
    )
    parser.add_argument(
        "--compile-all",
        action="store_true",
        help="Compile every latex/*.tex card (parallel).",
    )
    parser.add_argument(
        "--jobs",
        type=int,
